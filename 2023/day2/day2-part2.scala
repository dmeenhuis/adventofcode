package adventofcode2023.day2.part2
...
case class PlayedGame(id: Int, sets: Seq[GameSet]) {
  def isValid(config: GameConfiguration) = sets.forall(s => s.isValid(config))

  def minimumSetOfCubesPower() = {
    val (minRedCubes, minGreenCubes, minBlueCubes) = minNumberOfCubesRequired()
    minRedCubes * minGreenCubes * minBlueCubes
  }

  private def minNumberOfCubesRequired() =
    (sets.map(_.numRedCubes).max, sets.map(_.numGreenCubes).max, sets.map(_.numBlueCubes).max)
}
...
def calculateSumForFile(filename: String) = {
  ...
  source.getLines.map(parseGameFromLine).foldLeft(0){ case (acc, playedGame) =>
    acc + playedGame.minimumSetOfCubesPower()
  }
}
